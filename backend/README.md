...
2. **开夜间巩固线程** —— 每 `SOMA_CONSOLIDATE_INTERVAL` 秒（默认 6 小时）把当天案件蒸馏成记忆；
3. **建库** —— `soma.db`（SQLite，首次自动生成）。
...
| 变量 | 默认 | 作用 |
|---|---|---|
| `SOMA_API_KEY` | 空 | 杏仁核（初筛）模型的 key；**留空 = mock 模式**（零 key 先跑通架构） |
| `SOMA_BASE_URL` | `https://api.deepseek.com/v1` | OpenAI 兼容端点（DeepSeek / OpenRouter / Ollama / vLLM） |
| `SOMA_MODEL` | `deepseek-chat` | 杏仁核（便宜模型） |
| `SOMA_DEEP_MODEL` | `deepseek-reasoner` | 前额叶（贵模型，深想用；不想花钱换回 `deepseek-chat`） |
| `SOMA_SYSLOG_BIND` / `_PORT` | `0.0.0.0` / `5514` | syslog 接收地址 |
| `SOMA_CONSOLIDATE_INTERVAL` | `21600` | 夜间巩固间隔（秒） |
| `SOMA_API_TOKEN` | 空 | 可选共享 token；**设了则所有写操作（入库/处置/旋钮/清库）都要带 `X-API-Token`**，不设则本地免鉴权 |

---

## 五、鉴权（可选）

- 后端设 `SOMA_API_TOKEN` 后，写操作需要请求头 `X-API-Token: <token>`，否则 401。
- 前端会从 `localStorage` 读 `soma_token` 自动带上；本地用可在浏览器控制台设：
  `localStorage.setItem('soma_token', '<你的 token>')`。
...
| 文件 | 内容 |
|---|---|
| `backend/soma.db` | SQLite：alerts / artifacts / cases / reports / audit_logs |
